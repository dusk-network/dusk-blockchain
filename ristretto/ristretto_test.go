...
func TestEncodeHashToPoint(t *testing.T) {
	testPoints := []struct{ label, encodings string }{
	...
	for _, tv := range testPoints {
		// h.Reset()
		// h.Write([]byte(tv.label))
		var pt ristretto.Point
		// var buf [32]byte
		// copy(buf[:], h.Sum(nil))
		pt.Derive([]byte(tv.label))
		fmt.Println(hex.EncodeToString(pt.Bytes()))
