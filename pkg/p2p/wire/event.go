...
import (
	"bytes"
<<<<<<< Updated upstream:pkg/p2p/wire/event.go
=======
	"encoding/binary"
	"errors"
>>>>>>> Stashed changes:pkg/core/consensus/notary/event.go
...
type StepEventCollector map[uint8][]wire.Event
...
func (sec StepEventCollector) Contains(event wire.Event, step uint8) bool {
...
func (sec StepEventCollector) Store(event wire.Event, step uint8) int {
	eventList := sec[step]
...
func (a *committeeEventUnmarshaller) Unmarshal(r *bytes.Buffer, ev wire.Event) error {
	...
	committeeEvent, ok := ev.(*committeeEvent)
	if !ok {
