...
func generateOutputAddress(keyPair *key.Key, num int) ([]*key.StealthAddress, ristretto.Point) {
...
func GenerateInputs(_ byte, db *database.DB, totalAmount int64, key *key.Key) ([]*transactions.Input, int64, error) {
	...
	addresses, R := generateOutputAddress(key, numInputs)

	rand.Seed(time.Now().Unix())
	randAmount := rand.Int63n(totalAmount) + totalAmount/int64(numInputs) + 1 // [totalAmount/4 + 1, totalAmount*4]
