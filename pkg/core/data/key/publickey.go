...
func (pa PublicAddress) ToKey(netPrefix byte) (*PublicKey, error) {
	...
	_ = binary.Read(r, binary.BigEndian, &np)
	_ = binary.Read(r, binary.BigEndian, &publicSpendBytes)
	_ = binary.Read(r, binary.BigEndian, &publicViewBytes)
	_ = binary.Read(r, binary.BigEndian, &checksum)

	// check net prefix
	if np != netPrefix {
		return nil, errors.New("unrecognized network prefix")
	}

	// compare the checksum
	buf := new(bytes.Buffer)
	_ = buf.WriteByte(np)
	_, _ = buf.Write(publicSpendBytes[:])
	_, _ = buf.Write(publicViewBytes[:])
