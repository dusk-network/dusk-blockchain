...
func TestBlockAgreement(t *testing.T) {
	...
	votes, msgs, err := createVotesBlock(ctx, 10)
	if err != nil {
		t.Fatal(err)
	}
...
// Test for block agreement convenience function
func TestSendBlockAgreement(t *testing.T) {
	// Create context
	seed, _ := crypto.RandEntropy(32)
	keys, _ := user.NewRandKeys()
...
func createVotesBlock(ctx *user.Context, amount int) ([]*consensusmsg.Vote,
	...
	for _, c := range ctxs {
		...
		pl, err := consensusmsg.NewBlockAgreement(c.BlockHash, newVotes)
		if err != nil {
			return nil, nil, err
		}
		...
		msg, err := payload.NewMsgConsensus(c.Version, c.Round, c.LastHeader.Hash,
			c.BlockStep, sigEd, c.Keys.EdPubKeyBytes(), pl)
		if err != nil {
			return nil, nil, err
		}
