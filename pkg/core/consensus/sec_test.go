...
func TestClear(t *testing.T) {
	...
	require.Equal(t, 3, len(sec.findEvents(stepOne)))

	stepTwo := "2"
	sec.Store(ev1, stepTwo)
	sec.Store(ev2, stepTwo)
	sec.Store(ev3, stepTwo)
	require.Equal(t, 3, len(sec.findEvents(stepTwo)))

	sec.Clear()
	require.Equal(t, 0, len(sec.findEvents(stepOne)))
	require.Equal(t, 0, len(sec.findEvents(stepTwo)))
}
