...
func LaunchConsensus(ctx context.Context, eventBroker *eventbus.EventBus, rpcBus *rpcbus.RPCBus, w *wallet.Wallet, _ *chainsync.Counter, proxy *transactions.Proxy) {
	...
	if err := startProvisioner(ctx, eventBroker, rpcBus, w, proxy); err != nil {
		log.Panic(err)
	}
}

func startProvisioner(ctx context.Context, eventBroker *eventbus.EventBus, rpcBus *rpcbus.RPCBus, w *wallet.Wallet, proxy *transactions.Proxy) error {
...
func storeBidValues(eventBroker eventbus.Broker, rpcBus *rpcbus.RPCBus, w *wallet.Wallet) {
	// TODO: investigate if this is still needed after RUSK migration
	// If so, we need to rework this logic to be compatible
	/*
