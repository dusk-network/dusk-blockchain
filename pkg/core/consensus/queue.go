...
type Queue struct {
	lock    sync.RWMutex
	entries map[uint64]map[uint8][]TopicEvent
}
...
func NewQueue() *Queue {
	entries := make(map[uint64]map[uint8][]TopicEvent)
	return &Queue{
		entries: entries,
	}
}
...
func (eq *Queue) PutEvent(round uint64, step uint8, m TopicEvent) {
	...
	if eq.entries[round] == nil {
		eq.entries[round] = make(map[uint8][]TopicEvent)
	}

	// Initialise the array on this step if it was not yet created
	if eq.entries[round][step] == nil {
		eq.entries[round][step] = make([]TopicEvent, 0)
	}
