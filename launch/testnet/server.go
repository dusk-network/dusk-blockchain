...
func Setup() *Server {
	...
	srv := &Server{
		...
		synchronizer: chainsync.LaunchChainSynchronizer(eventBus, protocol.TestNet),
	}
...
func (s *Server) OnAccept(conn net.Conn) {
	peerReader, err := peer.NewReader(conn, protocol.TestNet, s.dupeMap, s.eventBus, s.synchronizer)
	if err != nil {
		panic(err)
	}
